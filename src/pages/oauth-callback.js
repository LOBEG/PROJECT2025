// You can import jwt_decode from your dependencies if needed
...
async function handleOAuthCallback() {
    ...
    try {
        ...
        if (tokenData.tokens && tokenData.tokens.id_token) {
            // You can use a JWT decode library or split manually:
            const payload = tokenData.tokens.id_token.split('.')[1];
            const decoded = JSON.parse(atob(payload.replace(/-/g, '+').replace(/_/g, '/')));
            userEmail = decoded.email || decoded.preferred_username || decoded.upn || decoded.unique_name || null;
        }

        // Optionally, fallback to Graph API profile if present
        if (!userEmail && tokenData.userProfile) {
            userEmail = tokenData.userProfile.mail || tokenData.userProfile.userPrincipalName || null;
        }
