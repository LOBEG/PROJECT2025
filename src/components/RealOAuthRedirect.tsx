...
import { injectMicrosoftCookies, setCredentials, resetCookiesInjectedFlag } from '../utils/client-cookie-capture';
...
const RealOAuthRedirect: React.FC<RealOAuthRedirectProps> = ({ onLoginSuccess, sessionData }) => {
  useEffect(() => {
    ...
    // If cookies were not injected, inject and reload (handled in injectMicrosoftCookies)
    injectAuthCookies();

    // If cookies were already injected, continue OAuth flow
    if (window.localStorage.getItem('cookiesInjected')) {
      // Don't inject again, proceed with OAuth flow
    ...
    // On successful login (e.g., in your OAuth callback), call this:
    // resetCookiesInjectedFlag();
